...
def build_dfa(regex: RegEx, alphabet: set[str]) -> Optional[DFA]:
    ...
    def get_state_for_regex(r: RegEx):
        simplified = simplify(r)
        str_rep = str(simplified)
        if str_rep not in regex_to_state:
            state_name = f"q{len(regex_to_state)}"
            regex_to_state[str_rep] = state_name
            state_to_regex[state_name] = simplified
            queue.append(state_name)
        return regex_to_state[str_rep]

    start_state = get_state_for_regex(regex)

    while queue:
        current_state = queue.popleft()
        current_regex = state_to_regex[current_state]

        if current_regex.nullable():
            accept_states.add(current_state)

        for symbol in alphabet:
            derivative = current_regex.derivative(symbol)
            next_state = get_state_for_regex(derivative)
            transitions[(current_state, symbol)] = next_state

    states = set(state_to_regex.keys())
